...
def test_eip_spec_version(
    ...
    if config is None:
        ...
        # A more robust solution might involve pytest_generate_tests or similar
        # if used outside of the defined EIPSpecTestItem context.
        pytest.fail(
    ...
    if not is_up_to_date:
        ...
        if hasattr(config, "_eip_version_failures"):
            ...
            print(
                "Warning: config._eip_version_failures not found. "
                "Failure data cannot be recorded."
            )
...
def generate_eip_report_markdown(failures: List[EIPVersionFailure], run_id: str) -> str:
    """
    Generates a markdown report for EIP version check failures.
    """
...
def pytest_sessionfinish(session):
    ...
    Generates a report if there are any EIP version failures.
    """
